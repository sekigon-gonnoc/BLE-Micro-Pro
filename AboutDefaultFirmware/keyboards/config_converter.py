    ...
    def parse_file(self, f):
        ...
        for string in cform:
            ...
            if 'VENDOR_ID' in string:
                self.vid = string.split('VENDOR_ID')[1].replace('\n', '').strip()
            elif 'PRODUCT_ID' in string:
                self.pid = string.split('PRODUCT_ID')[1].replace('\n', '').strip()
            elif 'MANUFACTURER' in string:
                self.manufacture = string.split('MANUFACTURER')[1].replace('\n', '').strip()
            elif 'PRODUCT' in string:
                self.name = string.split('PRODUCT')[1].replace('\n', '').strip()
            elif 'DESCRIPTION' in string:
                self.description = string.split('DESCRIPTION')[1].replace('\n', '').strip()
            elif re.match(r'\s*#\s*define\s+MATRIX_ROWS\s', string):
                if self.row_num != '':
                    self.waring_multiple_definition('MATRIX_ROWS', f, line)
                else:
                    self.row_num = string.split('MATRIX_ROWS')[1].replace('\n', '').strip()
            elif re.match(r'\s*#\s*define\s+MATRIX_COLS\s', string):
                if self.col_num != '':
                    self.waring_multiple_definition('MATRIX_COLS', f, line)
                else:
                    self.col_num = string.split('MATRIX_COLS')[1].replace('\n', '').strip()
                ...
                if self.row_pins_right != '':
                    self.waring_multiple_definition('MATRIX_ROW_PINS_RIGHT', f, line)
                else:
                ...
                if self.col_pins_right != '':
                    self.waring_multiple_definition('MATRIX_COL_PINS_RIGHT', f, line)
                else:
                ...
                if self.led_pin != '':
                    ...
                    pin = string.split('RGB_DI_PIN')[1].replace('\n', '').strip()
    ...
    with open(config_name, 'w') as f:
        f.write(
            config_file_format.format(
                config.vid, config.pid, config.name, config.manufacture, config.description, config.row_num, config.col_num, len(config.row_pins.split(',')), len(config.col_pins.split(',')), config.diode_direction, config.row_pins, config.col_pins,
    ...
    with open(config_name, 'w') as f:
        f.write(
            config_file_format.replace('"is_left_hand":1', '"is_left_hand":0').format(
                config.vid, config.pid, config.name, config.manufacture, config.description, config.row_num, config.col_num, len(config.row_pins_right.split(',')), len(config.col_pins_right.split(',')), config.diode_direction, config.row_pins_right,
