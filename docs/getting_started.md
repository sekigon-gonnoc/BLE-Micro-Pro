...
Google ChromeからBLE Micro Pro Web Configuratorにアクセスすればブラウザからファームウェアのアップデートや各種設定ができます。
...
校正を選択し終わったら`Next`ボタンを押すと次の手順に進みます。
...
- キーマップを設定する。
- `CONNECT BY SERIAL`の左側のボタンを押してキーボードに反映する。
